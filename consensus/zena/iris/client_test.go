...
func CreateMockIrisServer(wg *sync.WaitGroup, port int, listener net.Listener, handler *HttpHandlerFake) (*http.Server, error) {
...
// TestFetchCheckpointFromMockIris tests the iris client side logic
// to fetch checkpoints (latest for the scope of test) from a mock iris server.
// It can be used for debugging purpose (like response fields, marshalling/unmarshalling, etc).
func TestFetchCheckpointFromMockIris(t *testing.T) {
	t.Parallel()
	...
	// Create mock iris server and pass handler instance for setting up the routes
	srv, err := CreateMockIrisServer(wg, port, listener, handler)
	require.NoError(t, err, "expect no error in starting mock iris server")

	// Create a new iris client and use same port for connection
	client := NewIrisClient(fmt.Sprintf("http://localhost:%d", port))
	_, err = client.FetchCheckpoint(context.Background(), -1)
	require.NoError(t, err, "expect no error in fetching checkpoint")

	// Shutdown the server
	err = srv.Shutdown(context.TODO())
	require.NoError(t, err, "expect no error in shutting down mock iris server")

	// Wait for `wg.Done()` to be called in the mock server's routine.
	wg.Wait()
}

// TestFetchMilestoneFromMockIris tests the iris client side logic
// to fetch milestone from a mock iris server.
// It can be used for debugging purpose (like response fields, marshalling/unmarshalling, etc).
func TestFetchMilestoneFromMockIris(t *testing.T) {
	t.Parallel()
...
// TestFetchShutdown tests the iris client side logic for context timeout and
// interrupt handling while fetching data from a mock iris server.
func TestFetchShutdown(t *testing.T) {
	...
	// Case3 - Testing interrupt: Closing the `closeCh` in iris client simulating interrupt. This
...
func TestSpanURL(t *testing.T) {
	...
	url, err := spanURL("http://zena0", 1)
	if err != nil {
		t.Fatal("got an error", err)
	}

	const expected = "http://zena0/zena/span/1"
...
func TestStateSyncURL(t *testing.T) {
	...
	url, err := stateSyncURL("http://zena0", 10, 100)
	if err != nil {
		t.Fatal("got an error", err)
	}

	const expected = "http://zena0/clerk/event-record/list?from-id=10&to-time=100&limit=50"
