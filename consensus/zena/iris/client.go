...
var (
	...
	ErrNotSuccessfulResponse = errors.New("error while fetching data from Iris")
	ErrNotInRejectedList     = errors.New("milestoneID doesn't exist in rejected list")
	ErrNotInMilestoneList    = errors.New("milestoneID doesn't exist in Iris")
	ErrServiceUnavailable    = errors.New("service unavailable")
)

const (
	irisAPIBodyLimit = 128 * 1024 * 1024 // 128 MB
	stateFetchLimit  = 50
	apiIrisTimeout   = 5 * time.Second
...
type SpanResponse struct {
	Height string        `json:"height"`
	Result span.IrisSpan `json:"result"`
}
...
func NewIrisClient(urlString string) *IrisClient {
	return &IrisClient{
		urlString: urlString,
		client: http.Client{
			Timeout: apiIrisTimeout,
		},
...
const (
	...
	fetchSpanFormat = "zena/span/%d"
)

func (h *IrisClient) StateSyncEvents(ctx context.Context, fromID uint64, to int64) ([]*clerk.EventRecordWithTime, error) {
...
func (h *IrisClient) Span(ctx context.Context, spanID uint64) (*span.IrisSpan, error) {
...
// FetchCheckpoint fetches the checkpoint from iris
func (h *IrisClient) FetchCheckpoint(ctx context.Context, number int64) (*checkpoint.Checkpoint, error) {
...
// FetchMilestone fetches the checkpoint from iris
func (h *IrisClient) FetchMilestone(ctx context.Context) (*milestone.Milestone, error) {
...
// FetchCheckpointCount fetches the checkpoint count from iris
func (h *IrisClient) FetchCheckpointCount(ctx context.Context) (int64, error) {
...
// FetchMilestoneCount fetches the milestone count from iris
func (h *IrisClient) FetchMilestoneCount(ctx context.Context) (int64, error) {
...
// FetchLastNoAckMilestone fetches the last no-ack-milestone from iris
func (h *IrisClient) FetchLastNoAckMilestone(ctx context.Context) (string, error) {
...
// FetchNoAckMilestone fetches the last no-ack-milestone from iris
func (h *IrisClient) FetchNoAckMilestone(ctx context.Context, milestoneID string) error {
...
// to the given milestone is in process in Iris
func (h *IrisClient) FetchMilestoneID(ctx context.Context, milestoneID string) error {
...
// FetchWithRetry returns data from iris with retry
func FetchWithRetry[T any](ctx context.Context, client http.Client, url *url.URL, closeCh chan struct{}) (*T, error) {
	...
	// yet in iris. E.g. when the hardfork hasn't hit yet but iris
	// is upgraded.
	if errors.Is(err, ErrServiceUnavailable) {
		log.Debug("Iris service unavailable at the moment", "path", url.Path, "error", err)
		return nil, err
	}
	...
	log.Warn("an error while trying fetching from Iris", "path", url.Path, "attempt", attempt, "error", err)
...
retryLoop:
	for {
		log.Info("Retrying again in 5 seconds to fetch data from Iris", "path", url.Path, "attempt", attempt)
...
func internalFetch(ctx context.Context, client http.Client, u *url.URL) ([]byte, error) {
	...
	limitedBody := http.MaxBytesReader(nil, res.Body, irisAPIBodyLimit)
...
func internalFetchWithTimeout(ctx context.Context, client http.Client, url *url.URL) ([]byte, error) {
	ctx, cancel := context.WithTimeout(ctx, apiIrisTimeout)
	defer cancel()
