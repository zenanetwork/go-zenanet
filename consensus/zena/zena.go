...
// ecrecover extracts the Zenanet account address from a signed header.
func ecrecover(header *types.Header, sigcache *lru.ARCCache, c *params.ZenaConfig) (common.Address, error) {
	...
	// Recover the public key and the Zenanet address
...
func SealHash(header *types.Header, c *params.ZenaConfig) (hash common.Hash) {
...
func encodeSigHeader(w io.Writer, header *types.Header, c *params.ZenaConfig) {
	enc := []interface{}{
		header.ParentHash,
		header.UncleHash,
		header.Coinbase,
...
func CalcProducerDelay(number uint64, succession int, c *params.ZenaConfig) uint64 {
...
// ZenaRLP returns the rlp bytes which needs to be signed for the zena
...
func ZenaRLP(header *types.Header, c *params.ZenaConfig) []byte {
	b := new(bytes.Buffer)
...
type Zena struct {
	...
	authorizedSigner atomic.Pointer[signer] // Zenanet address and sign function of the signing key
...
type signer struct {
	signer common.Address // Zenanet address of the signing key
	signFn SignerFn       // Signer function to authorize hashes with
}

// New creates a Zena iris consensus engine.
func New(
	...
	// get zena config
	zenaConfig := chainConfig.Zena

	// Set any missing consensus parameters to their defaults
	if zenaConfig != nil && zenaConfig.CalculateSprint(0) == 0 {
		zenaConfig.Sprint = defaultSprintLength
	}
...
// Author implements consensus.Engine, returning the Zenanet address recovered
// from the signature in the header's extra-data section.
func (c *Zena) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures, c.config)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Zena) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
	return c.verifyHeader(chain, header, nil)
}

func (c *Zena) GetSpanner() Spanner {
	return c.spanner
}

func (c *Zena) SetSpanner(spanner Spanner) {
	c.spanner = spanner
}
...
func (c *Zena) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
func (c *Zena) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Zena) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
// nolint: gocognit
func (c *Zena) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Zena) VerifyUncles(_ consensus.ChainReader, block *types.Block) error {
...
func (c *Zena) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Zena) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func IsBlockOnTime(parent *types.Header, header *types.Header, number uint64, succession int, cfg *params.ZenaConfig) bool {
	return parent != nil && header.Time < parent.Time+CalcProducerDelay(number, succession, cfg)
}

// Prepare implements consensus.Engine, preparing all the consensus fields of the
// header for running the transactions on top.
func (c *Zena) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// rewards given.
func (c *Zena) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body) {
	...
	if IsSprintStart(headerNumber, c.config.CalculateSprint(headerNumber)) {
		start := time.Now()
		cx := statefull.ChainContext{Chain: chain, Zena: c}
...
func (c *Zena) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
	headerNumber := header.Number.Uint64()
	if body.Withdrawals != nil || header.WithdrawalsHash != nil {
...
// with.
func (c *Zena) Authorize(currentSigner common.Address, signFn SignerFn) {
	c.authorizedSigner.Store(&signer{
...
func (c *Zena) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
...
func Sign(signFn SignerFn, signer common.Address, header *types.Header, c *params.ZenaConfig) error {
	sighash, err := signFn(accounts.Account{Address: signer}, accounts.MimetypeZena, ZenaRLP(header, c))
	if err != nil {
		return err
	}
...
// current signer.
func (c *Zena) CalcDifficulty(chain consensus.ChainHeaderReader, _ uint64, parent *types.Header) *big.Int {
...
// SealHash returns the hash of a block prior to it being sealed.
func (c *Zena) SealHash(header *types.Header) common.Hash {
	return SealHash(header, c.config)
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the signer voting.
func (c *Zena) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "zena",
		Version:   "1.0",
		Service:   &API{chain: chain, zena: c},
		Public:    false,
	}}
}

// Close implements consensus.Engine. It's a noop for zena as there are no background threads.
func (c *Zena) Close() error {
	c.closeOnce.Do(func() {
		if c.IrisClient != nil {
			c.IrisClient.Close()
		}
...
func (c *Zena) checkAndCommitSpan(
	state *state.StateDB,
	header *types.Header,
...
func (c *Zena) needToCommitSpan(currentSpan *span.Span, headerNumber uint64) bool {
	// if span is nil
	if currentSpan == nil {
		return false
	}
...
func (c *Zena) FetchAndCommitSpan(
	ctx context.Context,
	newSpanID uint64,
	state *state.StateDB,
	header *types.Header,
	...
	var irisSpan span.IrisSpan

	if c.IrisClient == nil {
		// fixme: move to a new mock or fake and remove c.IrisClient completely
		s, err := c.getNextIrisSpanForTest(ctx, newSpanID, header, chain)
		if err != nil {
			return err
		}
		...
		response, err := c.IrisClient.Span(ctx, newSpanID)
		if err != nil {
			return err
		}
	...
	// check if chain id matches with Iris span
	if irisSpan.ChainID != c.chainConfig.ChainID.String() {
		return fmt.Errorf(
			"chain id proposed span, %s, and zena chain id, %s, doesn't match",
			irisSpan.ChainID,
			c.chainConfig.ChainID,
		)
	}

	return c.spanner.CommitSpan(ctx, irisSpan, state, header, chain)
}

// CommitStates commit states
func (c *Zena) CommitStates(
	state *state.StateDB,
	header *types.Header,
	...
	log.Info(
		"Fetching state updates from Iris",
		"fromID", from,
...
func (c *Zena) SetIrisClient(h IIrisClient) {
	c.IrisClient = h
}

func (c *Zena) GetCurrentValidators(ctx context.Context, headerHash common.Hash, blockNumber uint64) ([]*valset.Validator, error) {
...
func (c *Zena) getNextIrisSpanForTest(
	ctx context.Context,
	newSpanID uint64,
	header *types.Header,
	chain core.ChainContext,
	...
	spanZena, err := c.spanner.GetCurrentSpan(ctx, header.ParentHash)
	if err != nil {
		return nil, err
	}
	...
	// new span
	spanZena.ID = newSpanID
	if spanZena.EndBlock == 0 {
		spanZena.StartBlock = 256
	} else {
		spanZena.StartBlock = spanZena.EndBlock + 1
	}

	spanZena.EndBlock = spanZena.StartBlock + (100 * c.config.CalculateSprint(headerNumber)) - 1
