...
func loadSnapshot(chainConfig *params.ChainConfig, config *params.ZenaConfig, sigcache *lru.ARCCache, db ethdb.Database, hash common.Hash) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("zena-"), hash[:]...))
	if err != nil {
		return nil, err
	}
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("zena-"), s.Hash[:]...), blob)
}
...
func (s *Snapshot) apply(headers []*types.Header, c *Zena) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		if number >= s.chainConfig.Zena.CalculateSprint(number) {
			delete(snap.Recents, number-s.chainConfig.Zena.CalculateSprint(number))
		}

		// Resolve the authorization key and check against signers
		signer, err := ecrecover(header, s.sigcache, s.chainConfig.Zena)
		if err != nil {
			return nil, err
		}
		...
		if number > 0 && (number+1)%s.chainConfig.Zena.CalculateSprint(number) == 0 {
