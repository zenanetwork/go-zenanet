...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.zena.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetAuthor(blockNrOrHash *rpc.BlockNumberOrHash) (*common.Address, error) {
	...
	author, err := api.zena.Author(header)
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.zena.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
