...
func generateTestBlock(number int, txs []*types.Transaction) *types.Block {
	...
	// 참고: 실제 구현에서는 types.NewBlock의 인터페이스에 맞게 수정 필요
	// 여기서는 테스트 목적으로 간단히 구현
	body := &types.Body{Transactions: txs, Uncles: []*types.Header{}}
...
func TestBatchProcessingPerformance(t *testing.T) {
	...
	for _, batchSize := range batchSizes {
		// 배치 처리기 생성
		processor := NewStateBatchProcessor(batchSize)
