...
func (s *BlockChainAPI) GetZenaBlockReceipt(ctx context.Context, hash common.Hash) (*types.Receipt, error) {
	return s.b.GetZenaBlockReceipt(ctx, hash)
}
...
//
// Zena transaction utils
//

func (s *BlockChainAPI) appendRPCMarshalZenaTransaction(ctx context.Context, block *types.Block, fields map[string]interface{}, fullTx bool) map[string]interface{} {
	if block != nil {
		txHash := types.GetDerivedZenaTxHash(types.ZenaReceiptKey(block.Number().Uint64(), block.Hash()))

		ZenaTx, blockHash, blockNumber, txIndex, _ := s.b.GetZenaBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
		if ZenaTx != nil {
			formattedTxs := fields["transactions"].([]interface{})

			if fullTx {
				marshalledTx := newRPCTransaction(ZenaTx, blockHash, blockNumber, block.Time(), txIndex, block.BaseFee(), s.b.ChainConfig())
...
// ZenaAPI provides an API to access Zena related information.
type ZenaAPI struct {
	b Backend
}

// NewZenaAPI creates a new Zena protocol API.
func NewZenaAPI(b Backend) *ZenaAPI {
	return &ZenaAPI{b}
}
...
func (api *ZenaAPI) SendRawTransactionConditional(ctx context.Context, input hexutil.Bytes, options types.OptionsPIP15) (common.Hash, error) {
...
func (api *ZenaAPI) GetVoteOnHash(ctx context.Context, starBlockNr uint64, endBlockNr uint64, hash string, milestoneId string) (bool, error) {
