...
import (
	...
	"github.com/zenanetwork/go-zenanet/eth/gasestimator"
...
func NewZenanetAPI(b Backend) *ZenanetAPI {
	return &ZenanetAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (api *ZenanetAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (api *ZenanetAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := api.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (api *ZenanetAPI) FeeHistory(ctx context.Context, blockCount math.HexOrDecimal64, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (api *ZenanetAPI) BlobBaseFee(ctx context.Context) *hexutil.Big {
...
func (api *ZenanetAPI) Syncing() (interface{}, error) {
...
type ZenanetAccountAPI struct {
	am *accounts.Manager
}

// NewZenanetAccountAPI creates a new ZenanetAccountAPI.
func NewZenanetAccountAPI(am *accounts.Manager) *ZenanetAccountAPI {
	return &ZenanetAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (api *ZenanetAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Zenanet ECDSA signature for:
// keccak256("\x19Zenanet Signed Message:\n" + len(message) + message))
//
...
//
// https://gzen.ethereum.org/docs/interacting-with-gzen/rpc/ns-personal#personal-sign
...
// the address of:
// hash = keccak256("\x19Zenanet Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://gzen.ethereum.org/docs/interacting-with-gzen/rpc/ns-personal#personal-ecrecover
func (api *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, errors.New("invalid Zenanet signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gzen.
...
func (api *BlockChainAPI) GetTransactionReceiptsByBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) ([]map[string]interface{}, error) {
	...
	zenaReceipt, err := api.b.GetZenaBlockReceipt(ctx, block.Hash())
	if err != nil && err != zenanet.NotFound {
		return nil, err
	}
	if zenaReceipt != nil {
		receipts = append(receipts, zenaReceipt)

		txHash = types.GetDerivedZenaTxHash(types.ZenaReceiptKey(block.Number().Uint64(), block.Hash()))
		if txHash != (common.Hash{}) {
			zenaTx, _, _, _, _ := api.b.GetZenaBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
			txs = append(txs, zenaTx)
		}
	}
	...
	for idx, receipt := range receipts {
		...
		if zenaReceipt != nil && idx == len(receipts)-1 {
...
func (api *BlockChainAPI) GetBlockByNumber(ctx context.Context, number rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {
	...
	if block != nil && err == nil {
		...
		// append marshalled zena transaction
		if err == nil && response != nil {
			response = api.appendRPCMarshalZenaTransaction(ctx, block, response, fullTx)
		}
...
func (api *BlockChainAPI) GetBlockByHash(ctx context.Context, hash common.Hash, fullTx bool) (map[string]interface{}, error) {
	...
	if block != nil {
		...
		// append marshalled zena transaction
		if err == nil && response != nil {
			return api.appendRPCMarshalZenaTransaction(ctx, block, response, fullTx), err
		}
...
func (api *BlockChainAPI) GetBlockReceipts(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) ([]map[string]interface{}, error) {
	...
	stateSyncReceipt, err := api.b.GetZenaBlockReceipt(ctx, block.Hash())
	if err != nil && err != zenanet.NotFound {
		return nil, err
	}
	if stateSyncReceipt != nil {
		tx, _, _, _ := rawdb.ReadZenaTransaction(api.b.ChainDb(), stateSyncReceipt.TxHash)
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, state *state.StateDB, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	// ZENA: This is used by zena consensus to fetch data from genesis contracts for state-sync
	...
	if state == nil {
		...
		// when we're doing a call from zena consensus to fetch data from genesis contracts. It's
...
func newRPCTransactionFromBlockIndex(b *types.Block, index uint64, config *params.ChainConfig, db ethdb.Database) *RPCTransaction {
	...
	var zenaReceipt *types.Receipt

	// Read zena receipts if a state-sync transaction is requested
	if index == uint64(len(txs)) {
		zenaReceipt = rawdb.ReadZenaReceipt(db, b.Hash(), b.NumberU64(), config)
		if zenaReceipt != nil {
			if zenaReceipt.TxHash != (common.Hash{}) {
				zenaTx, _, _, _ := rawdb.ReadZenaTransactionWithBlockHash(db, zenaReceipt.TxHash, b.Hash())
				if zenaTx != nil {
					txs = append(txs, zenaTx)
				}
			}
	...
	// If the index is still out of the range after checking zena state sync transaction, it means that the transaction index is invalid
	if index >= uint64(len(txs)) {
		return nil
	}
	...
	// If the transaction is a zena transaction, we need to set the hash to the derived zena tx hash. ZenaTx is always the last index.
	if zenaReceipt != nil && index == uint64(len(txs)-1) {
		rpcTx.Hash = zenaReceipt.TxHash
		rpcTx.ChainID = nil
	}
...
func (api *TransactionAPI) getAllBlockTransactions(ctx context.Context, block *types.Block) (types.Transactions, bool) {
	...
	zenaReceipt, _ := api.b.GetZenaBlockReceipt(ctx, block.Hash())
	if zenaReceipt != nil {
		txHash := types.GetDerivedZenaTxHash(types.ZenaReceiptKey(block.Number().Uint64(), block.Hash()))
		if txHash != (common.Hash{}) {
			zenaTx, _, _, _, _ := api.b.GetZenaBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
...
func (api *TransactionAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	// fetch zena block tx if necessary
	if !found {
		if tx, blockHash, blockNumber, index, err = api.b.GetZenaBlockTransaction(ctx, hash); err != nil {
			return nil, err
		}
	...
	if tx != nil {
		...
		if zenaTx {
			...
			// In case of zena block tx, we need simple derived tx hash (same as function argument) instead of RLP hash
			resultTx.Hash = hash
...
func (api *TransactionAPI) GetTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	...
	if !found {
		tx, blockHash, blockNumber, index, err = api.b.GetZenaBlockTransaction(ctx, hash)
		if err != nil {
			return nil, err
		}
		zenaTx = true
	...
	if zenaTx {
		// Fetch zena block receipt
		receipt, err = api.b.GetZenaBlockReceipt(ctx, blockHash)
		if err != nil && err != zenanet.NotFound {
			return nil, err
		}
	} else {
	...
	return marshalReceipt(receipt, blockHash, blockNumber, signer, tx, int(index), zenaTx), nil
}

// marshalReceipt marshals a transaction receipt into a JSON object.
func marshalReceipt(receipt *types.Receipt, blockHash common.Hash, blockNumber uint64, signer types.Signer, tx *types.Transaction, txIndex int, zenaTx bool) map[string]interface{} {
	from, _ := types.Sender(signer, tx)

	var txHash common.Hash

	if zenaTx {
		txHash = types.GetDerivedZenaTxHash(types.ZenaReceiptKey(blockNumber, blockHash))
	} else {
		txHash = tx.Hash()
	}
...
//
// https://github.com/zenanetwork/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Zenanet APIs exposed over the debugging
// namespace.
...
// Version returns the current zenanet protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Zen)))

	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f zen) exceeds the configured cap (%.2f zen)", feeFloat, cap)
	}
