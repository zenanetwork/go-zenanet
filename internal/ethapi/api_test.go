...
import (
	...
	"github.com/zenanetwork/go-zenanet/internal/ethapi/override"
...
func TestEstimateGas(t *testing.T) {
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				accounts[2].addr: {Balance: big.NewInt(params.Zen), Code: append(types.DelegationPrefix, accounts[3].addr.Bytes()...)},
			},
		}
		genBlocks      = 10
	...
	var testSuite = []struct {
		...
		},
		{
			...
			call:        TransactionArgs{},
			overrides: override.StateOverride{
				randomAccounts[0].addr: override.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Zen)))},
			},
			expectErr: nil,
			want:      53000,
		},
	...
	var (
		...
		genesis      = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				//     }
				// }
				cac: {Balance: big.NewInt(params.Zen), Code: common.Hex2Bytes("610dad80ff")},
				bab: {
...
func TestSimulateV1ChainLinkage(t *testing.T) {
	var (
		...
		gspec        = &core.Genesis{
			Config: params.MergedTestChainConfig,
			Alloc: types.GenesisAlloc{
				sender:       {Balance: big.NewInt(params.Zen)},
