...
import (
	...
	"github.com/zenanetwork/go-zenanet/internal/cli/server/chains"
...
type Config struct {
	...
	// state.scheme selects the Scheme to use for storing zenanet state ('hash' or 'path')
	...
	// ZenaLogs enables zena log retrieval
	ZenaLogs bool `hcl:"zena.logs,optional" toml:"zena.logs,optional"`
...
type SealerConfig struct {
	...
	// Zenbase is the address of the validator
	Zenbase string `hcl:"zenbase,optional" toml:"zenbase,optional"`
...
type AccountsConfig struct {
	...
	// DisableZenaWallet disables the personal wallet endpoints
	DisableZenaWallet bool `hcl:"disable-zena-wallet,optional" toml:"disable-zena-wallet,optional"`
}
...
func DefaultConfig() *Config {
	return &Config{
		...
		TxPool: &TxPoolConfig{
			...
			PriceLimit:   params.ZenaDefaultTxPoolPriceLimit, // zena's default
			PriceBump:    10,
			AccountSlots: 16,
			GlobalSlots:  131072,
			AccountQueue: 64,
		...
		Sealer: &SealerConfig{
			Enabled:             false,
			Zenbase:             "",
			GasCeil:             30_000_000,                                  // gzen's default
			GasPrice:            big.NewInt(params.ZenaDefaultMinerGasPrice), // zena's default
			ExtraData:           "",
		...
		Gpo: &GpoConfig{
			...
			IgnorePrice:      gasprice.DefaultIgnorePrice, // zena's default
		},
		JsonRPC: &JsonRPCConfig{
			...
			Http: &APIConfig{
				...
				Prefix:                      "",
				Host:                        "localhost",
				API:                         []string{"eth", "net", "web3", "txpool", "zena"},
...
func (c *Config) buildEth(stack *node.Node, accountManager *accounts.Manager) (*ethconfig.Config, error) {
	...
	// miner options
	{
		...
		if zenbase := c.Sealer.Zenbase; zenbase != "" {
			if !common.IsHexAddress(zenbase) {
				return nil, fmt.Errorf("zenbase is not an address: %s", zenbase)
			}

			n.Miner.Zenbase = common.HexToAddress(zenbase)
		}
	}
	...
	if c.Developer.Enabled {
		...
		// zenbase has been set above, configuring the miner address from command line flags.
		if n.Miner.Zenbase != (common.Address{}) {
			developer = accounts.Account{Address: n.Miner.Zenbase}
		...
		// Set the Zenbase
		c.Sealer.Zenbase = developer.Address.Hex()
		n.Miner.Zenbase = developer.Address
	...
	case "snap":
		...
		log.Warn("Zena doesn't support Snap Sync yet, switching to Full Sync mode")
	default:
...
func (c *Config) buildNode() (*node.Config, error) {
	...
	if c.Developer.Enabled {
		...
		cfg.HTTPModules = []string{"admin", "debug", "eth", "miner", "net", "personal", "txpool", "web3", "zena"}
	}
...
func DefaultDataDir() string {
	...
	switch runtime.GOOS {
	case "darwin":
		return filepath.Join(home, "Library", "Zena")
	case "windows":
		...
		return filepath.Join(appdata, "Zena")
	default:
		return filepath.Join(home, ".zena")
	}
}
