...
func TestConfigDefault(t *testing.T) {
	...
	assert.NoError(t, err)
	assertZenaDefaultGasPrice(t, ethConfig)
}

// assertZenaDefaultGasPrice asserts the zena default gas price is set correctly.
func assertZenaDefaultGasPrice(t *testing.T, ethConfig *ethconfig.Config) {
	assert.NotNil(t, ethConfig)
	assert.Equal(t, ethConfig.Miner.GasPrice, big.NewInt(params.ZenaDefaultMinerGasPrice))
}
