...
func TestFlagsWithoutConfig(t *testing.T) {
	...
	require.Equal(t, c.config.ZenaLogs, false)
	...
	require.Equal(t, c.config.JsonRPC.Http.API, []string{"eth", "zena"})
...
func TestFlagsWithConfigAndFlags(t *testing.T) {
	...
	require.Equal(t, c.config.ZenaLogs, true)
	...
	require.Equal(t, c.config.JsonRPC.Ws.API, []string{"eth", "zena", "web3"})
