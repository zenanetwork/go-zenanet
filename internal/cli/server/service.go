...
import (
	...
	"github.com/zenanetwork/go-zenanet/internal/cli/server/pprof"
...
var ErrUnavailable = errors.New("zena service is currently unavailable, try again later")
var ErrUnavailable2 = errors.New("zena service unavailable even after waiting for 10 seconds, make sure zena is running")

func sendStreamDebugFile(stream proto.Zena_DebugPprofServer, headers map[string]string, data []byte) error {
...
func (s *Server) DebugPprof(req *proto.DebugPprofRequest, stream proto.Zena_DebugPprofServer) error {
	var (
		payload []byte
...
func (s *Server) Status(ctx context.Context, in *proto.StatusRequest) (*proto.StatusResponse, error) {
	...
	resp := &proto.StatusResponse{
		...
		Forks: gatherForks(s.config.chain.Genesis.Config, s.config.chain.Genesis.Config.Zena),
	}
...
func (s *Server) DebugBlock(req *proto.DebugBlockRequest, stream proto.Zena_DebugBlockServer) error {
	...
	res, err := s.tracerAPI.TraceZenaBlock(traceReq)
	if err != nil {
		return err
	}
...
func (s *Server) ChainWatch(req *proto.ChainWatchRequest, reply proto.Zena_ChainWatchServer) error {
