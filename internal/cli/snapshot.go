...
import (
	...
	"github.com/zenanetwork/go-zenanet/internal/cli/server"
...
func (c *PruneStateCommand) MarkDown() string {
	items := []string{
		`# Prune state`,
		`The ` + "```" + "zena snapshot prune-state" + "```" + ` command will prune historical state data
...
func (c *PruneStateCommand) Help() string {
	return `Usage: zena snapshot prune-state <datadir>
...
func (c *PruneBlockCommand) MarkDown() string {
	items := []string{
		...
		"The ```zena snapshot prune-block``` command will prune historical blockchain data stored in the ancientdb. The amount of blocks expected for remaining after prune can be specified via `block-amount-reserved` in this command, will prune and only remain the specified amount of old block data in ancientdb.",
		`
...
func (c *PruneBlockCommand) Help() string {
	return `Usage: zena snapshot prune-block <datadir>
...
func (c *PruneBlockCommand) pruneBlock(stack *node.Node, fdHandles int) error {
	...
	if _, err := os.Stat(newAncientPath); err == nil {
		// No file lock found for old ancientDB but new ancientDB exsisted, indicating the gzen was interrupted
...
func (c *InspectAncientDbCommand) MarkDown() string {
	items := []string{
		...
		"The ```zena snapshot inspect-ancient-db``` command will inspect few fields in the ancient datastore using the given datadir location.",
		`
...
func (c *InspectAncientDbCommand) Help() string {
	return `Usage: zena snapshot inspect-ancient-db <datadir>
