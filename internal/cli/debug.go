...
func (c *DebugCommand) MarkDown() string {
	examples := []string{
		...
		CodeBlock([]string{
			"$ zena debug",
			"Starting debugger...\n",
			"Created debug archive: zena-debug-2021-10-26-073819Z.tar.gz",
		}),
		"Send the output to a specific directory:",
		CodeBlock([]string{
			"$ zena debug --output data",
			"Starting debugger...\n",
			"Created debug directory: data/zena-debug-2021-10-26-075437Z",
		}),
	}

	items := []string{
		"# Debug",
		"The ```zena debug``` command takes a debug dump of the running client.",
		"- [```zena debug pprof```](./debug_pprof.md): Dumps zena pprof traces.",
		"- [```zena debug block <number>```](./debug_block.md): Dumps zena block traces.",
	}
