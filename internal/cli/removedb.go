...
func (c *RemoveDBCommand) MarkDown() string {
	items := []string{
		"# RemoveDB",
		"The ```zena removedb``` command will remove the blockchain and state databases at the given datadir location",
		c.Flags().MarkDown(),
	}
...
func (c *RemoveDBCommand) Help() string {
	return `Usage: zena removedb <datadir>
...
func (c *RemoveDBCommand) Run(args []string) int {
	...
	// create zenanet node config with just the datadir
