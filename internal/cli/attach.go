...
// AttachCommand is the command to Connect to remote Zena IPC console
...
func (c *AttachCommand) MarkDown() string {
	items := []string{
		"# Attach",
		"Connect to remote Zena IPC console.",
		c.Flags().MarkDown(),
	}
...
func (c *AttachCommand) Help() string {
	return `Usage: zena attach <IPC FILE>
...
func (c *AttachCommand) Synopsis() string {
	return "Connect to Zena via IPC"
}
...
// remoteConsole will connect to a remote zena instance, attaching a JavaScript
// console to it.
// nolint: unparam
func (c *AttachCommand) remoteConsole() error {
	// Attach to a remotely running gzen instance and start the JavaScript console
	...
	if c.Endpoint == "" {
		...
		c.Endpoint = fmt.Sprintf("%s/zena.ipc", path)
	}

	client, err := dialRPC(c.Endpoint)

	if err != nil {
		utils.Fatalf("Unable to attach to remote zena: %v", err)
	}
...
// for "gzen attach" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint("zena")
