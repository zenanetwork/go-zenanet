...
import (
	...
	"github.com/zenanetwork/go-zenanet/version"
)

const ourPath = "github.com/zenanetwork/go-zenanet" // Path to our module
...
// Archive holds the textual version string used for Gzen archives. e.g.
...
// conventions in the Zenanet p2p network.
...
//
// If the package that is currently executing is a prefixed by our go-zenanet
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gzen".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gzen %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gzen %s", WithMeta)
	}
