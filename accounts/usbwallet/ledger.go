...
// https://github.com/LedgerHQ/app-zenanet/blob/develop/doc/ethapp.adoc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Zenanet address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Zenanet transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs an Zenanet message following the EIP 712 specification
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Zenanet app in browser mode", w.failure
	}

	if w.offline() {
		return "Zenanet app offline", w.failure
	}

	return fmt.Sprintf("Zenanet app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Zenanet app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Zenanet app is not running or in browser mode, nothing more to do, return
	...
	// Try to resolve the Zenanet app's version, will fail prior to v1.0.2
...
// and returning the Zenanet address located on that derivation path.
...
//
// Note, if the version of the Zenanet application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Zenanet app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Zenanet wallet app running
...
// ledgerDerive retrieves the currently active Zenanet address from a Ledger
...
//	Zenanet address length | 1 byte
//	Zenanet address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Zenanet hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
	...
	// Decode the hex string into an Zenanet address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// https://github.com/LedgerHQ/app-zenanet/issues/409
	chunk := 255
	...
	// Extract the Zenanet signature and do a sanity validation
