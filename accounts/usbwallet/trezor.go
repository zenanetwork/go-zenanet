...
import (
	...
	"github.com/zenanetwork/go-zenanet/accounts/usbwallet/trezor"
...
// or whether the Zenanet app was not started on it.
...
// Zenanet address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.ZenanetAddress)
	if _, err := w.trezorExchange(&trezor.ZenanetGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.ZenanetTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.ZenanetTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
