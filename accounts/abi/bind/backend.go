...
type ContractCaller interface {
	...
	// CallContract executes an Zenanet contract call with the specified data as the
	// input.
	CallContract(ctx context.Context, call zenanet.CallMsg, blockNumber *big.Int) ([]byte, error)
}
...
type PendingContractCaller interface {
	...
	// PendingCallContract executes an Zenanet contract call against the pending state.
	PendingCallContract(ctx context.Context, call zenanet.CallMsg) ([]byte, error)
}
...
type BlockHashContractCaller interface {
	...
	// CallContractAtHash executes an Zenanet contract call against the state at the specified block hash.
	CallContractAtHash(ctx context.Context, call zenanet.CallMsg, blockHash common.Hash) ([]byte, error)
}
...
type ContractTransactor interface {
	zenanet.GasEstimator
	zenanet.GasPricer
	zenanet.GasPricer1559
	zenanet.TransactionSender
