// Copyright 2019 The go-zenanet Authors
...
import (
	...
	"github.com/zenanetwork/go-zenanet/common/leak"
...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != zenanet.NotFound {
		t.Fatalf("err should be `zenanet.NotFound` but received %v", err)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	*/
	...
	sim := NewSimulatedBackend(types.GenesisAlloc{addr: {Balance: big.NewInt(params.Zen)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		name        string
		message     zenanet.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", zenanet.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", zenanet.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(types.GenesisAlloc{addr: {Balance: big.NewInt(params.Zen*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", zenanet.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", zenanet.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", zenanet.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(1e14), // gascost = 2.1zen
			Value:    big.NewInt(1e17), // the remaining balance for fee is 2.1zen
			Data:     nil,
		}, 21000, nil},

		{"EstimateWithSuperhighPrice", zenanet.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(2e14), // gascost = 4.2zen
			Value:    big.NewInt(100000000000),
			Data:     nil,
		}, 21000, errors.New("gas required exceeds allowance (10999)")}, // 10999=(2.2zen-1000wei)/(2e14)

		{"EstimateEIP1559WithHighFees", zenanet.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1zen
			GasTipCap: big.NewInt(1),
			Value:     big.NewInt(1e17), // the remaining balance for fee is 2.1zen
			Data:      nil,
		}, params.TxGas, nil},

		{"EstimateEIP1559WithSuperHighFees", zenanet.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1zen
			GasTipCap: big.NewInt(1),
			Value:     big.NewInt(1e17 + 1), // the remaining balance for fee is 2.1zen
			Data:      nil,
		}, params.TxGas, errors.New("gas required exceeds allowance (20999)")}, // 20999=(2.2zen-0.1zen-1wei)/(1e14)
	}
...
func TestPendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, zenanet.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, zenanet.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
	...
	res, err = sim.CallContractAtHash(bgCtx, zenanet.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, blockHash)
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, zenanet.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, zenanet.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
