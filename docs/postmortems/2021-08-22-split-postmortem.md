...
This is a post-mortem concerning the minority split that occurred on Zenanet mainnet on block [13107518](https://zenscan.io/block/13107518), at which a minority chain split occurred.
...
- 2021-08-24: Released [v1.10.8](https://github.com/zenanetwork/go-zenanet/releases/tag/v1.10.8) containing the fix on Tuesday morning (CET). Erigon released [v2021.08.04](https://github.com/ledgerwatch/erigon/releases/tag/v2021.08.04).
...
On 2021-08-17, Guido Vranken submitted a report to bounty@ethereum.org. This coincided with a gzen-meetup in Berlin, so the gzen team could fairly quickly analyse the issue. 
...
On the evening of 17th, we discussed options on how to handle it. We made a state test to reproduce the issue, and verified that neither `openzenanet`, `nzenmind` nor `besu` were affected by the same vulnerability, and started a full-sync with a patched version of `gzen`. 
...
- Place the fix into the PR optimizing the jumpdest analysis [233381](https://github.com/zenanetwork/go-zenanet/pull/23381). 
- After 4-8 weeks, release details about the vulnerability. 


## Exploit

At block [13107518](https://zenscan.io/block/13107518), mined at Aug-27-2021 12:50:07 PM +UTC, a minority chain split occurred. The discord user @AlexSSD7 notified the allcoredevs-channel on the Eth R&D discord, on Aug 27 13:09  UTC. 
...
The gzen-team have an official policy regarding [vulnerability disclosure](https://gzen.ethereum.org/docs/developers/gzen-developer/disclosures). 

> The primary goal for the Gzen team is the health of the Zenanet network as a whole, and the decision whether or not to publish details about a serious vulnerability boils down to minimizing the risk and/or impact of discovery and exploitation.
...
Action point: create a low-volume gzen-announce@ethereum.org email list where dependent projects/operators can receive public announcements. 
- This has been done. If you wish to receive release- and security announcements, sign up [here](https://groups.google.com/a/ethereum.org/g/gzen-announce/about)
...
- [1] https://twitter.com/go_zenanet/status/1428051458763763721
...
```
We have identified a security issue with go-zenanet, and will issue a
...
upgrade to the latest go-zenanet codebase. More information about the
issue will be disclosed at a later date.

https://twitter.com/go_zenanet/status/1428051458763763721
