...
func TestTxIndexer(t *testing.T) {
	...
	zenaReceipts := make([]types.Receipts, len(receipts))

	for _, c := range cases {
		frdir := t.TempDir()
		db, _ := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), frdir, "", false, false, false)
		_, _ = rawdb.WriteAncientBlocks(db, append([]*types.Block{gspec.ToBlock()}, blocks...), append([]types.Receipts{{}}, receipts...), append([]types.Receipts{{}}, zenaReceipts...), big.NewInt(0))
