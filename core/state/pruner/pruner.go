...
func (p *BlockPruner) backupOldDb(name string, cache, handles int, namespace string, readonly, interrupt bool) error {
	...
	writeBlock := func(blockNumber uint64) error {
		...
		zenaReceipts := []*types.Receipt{rawdb.ReadRawZenaReceipt(chainDb, blockHash, blockNumber)}
		...
		if _, err := rawdb.WriteAncientBlocks(frdbBack, []*types.Block{block}, []types.Receipts{receipts}, []types.Receipts{zenaReceipts}, td); err != nil {
