...
// https://github.com/zenanetwork/go-zenanet/pull/15549.
...
func TestMVHashMapReadWriteDelete(t *testing.T) {
	...
	states[1].SetBalance(addr, balance, tracing.BalanceChangeTransfer)
...
func TestMVHashMapRevert(t *testing.T) {
	...
	states[0].SetBalance(addr, balance, tracing.BalanceChangeTransfer)
	states[0].FlushMVWriteSet()

	// Tx1 perform some ops and then revert
	snapshot := states[1].Snapshot()
	states[1].AddBalance(addr, uint256.NewInt(100), tracing.BalanceChangeTransfer)
...
func TestMVHashMapOverwrite(t *testing.T) {
	...
	states[0].SetBalance(addr, balance1, tracing.BalanceChangeTransfer)
	states[0].FlushMVWriteSet()

	// Tx1 write
	states[1].SetState(addr, key, val2)
	states[1].SetBalance(addr, balance2, tracing.BalanceChangeTransfer)
	v := states[1].GetState(addr, key)
	b := states[1].GetBalance(addr)
	states[1].FlushMVWriteSet()
...
func TestMVHashMapWriteNoConflict(t *testing.T) {
	...
	states[1].SetBalance(addr, balance1, tracing.BalanceChangeTransfer)
	states[1].FlushMVWriteSet()
...
func TestApplyMVWriteSet(t *testing.T) {
	...
	sSingleProcess.SetBalance(addr1, balance1, tracing.BalanceChangeTransfer)
...
// See https://github.com/zenanetwork/go-zenanet/pull/15225#issuecomment-380191512
...
//
// See https://github.com/zenanetwork/go-zenanet/issues/20106.
