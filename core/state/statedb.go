...
// Package state provides a caching layer atop the Zenanet state trie.
package state
...
import (
	...
	"github.com/zenanetwork/go-zenanet/trie/triestate"
...
// StateDB structs within the zenanet protocol are used to store anything
...
type StateDB struct {
	...
	// Zena metrics
	ZenaConsensusTime time.Duration
...
func (s *StateDB) StartPrefetcher(namespace string, witness *stateless.Witness) {
	...
	// If snapshots are enabled, start prefzens explicitly
	if s.snap != nil {
		...
		// https://github.com/zenanetwork/go-zenanet/issues/29880
...
func (s *StateDB) Finalise(deleteEmptyObjects bool) {
	...
	for addr := range s.journal.dirties {
		...
		if obj.selfDestructed || (deleteEmptyObjects && obj.empty()) {
			...
			// If zen was sent to account post-selfdestruct it is burnt.
