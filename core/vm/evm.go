...
type BlockContext struct {
	...
	// sufficient zen to transfer the value
	CanTransfer CanTransferFunc
	// Transfer transfers zen from one account to the other
...
// EVM is the Zenanet Virtual Machine base object and provides
...
type EVM struct {
	...
	// evm.
	Config Config
	// global (to this context) zenanet virtual machine
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// Fail if we're trying to transfer more than the available balance
	// Note although it's noop to transfer X zen to caller itself. But
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *uint256.Int, address common.Address, typ OpCode) (ret []byte, createAddress common.Address, leftOverGas uint64, err error) {
	...
	if err == nil && evm.chainRules.IsEIP158 {
		if evm.chainConfig.Zena != nil && evm.chainConfig.Zena.IsAhmedabad(evm.Context.BlockNumber) {
