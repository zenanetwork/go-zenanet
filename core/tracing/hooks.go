...
// invoked by the go-zenanet core at various points in the state transition.
//
// To create a tracer that can be invoked with Gzen, you need to register it using
// [github.com/zenanetwork/go-zenanet/eth/tracers.LiveDirectory.Register].
//
// See https://gzen.ethereum.org/docs/developers/evm-tracing/live-tracing for a tutorial.
package tracing
...
const (
	...
	// BalanceIncreaseWithdrawal is zen withdrawn from the beacon chain.
	BalanceIncreaseWithdrawal BalanceChangeReason = 3
	// BalanceIncreaseGenesisBalance is zen allocated at the genesis block.
	...
	// BalanceIncreaseGasReturn is zen returned for unused gas at the end of execution.
	BalanceIncreaseGasReturn BalanceChangeReason = 7

	// DAO fork
	// BalanceIncreaseDaoContract is zen sent to the DAO refund contract.
	BalanceIncreaseDaoContract BalanceChangeReason = 8
	// BalanceDecreaseDaoAccount is zen taken from a DAO account to be moved to the refund contract.
	BalanceDecreaseDaoAccount BalanceChangeReason = 9

	// BalanceChangeTransfer is zen transferred via a call.
	...
	// BalanceDecreaseSelfdestructBurn is zen that is sent to an already self-destructed
