...
func (f *chainFreezer) freezeRange(nfdb *nofreezedb, number, limit uint64) (hashes []common.Hash, err error) {
	...
	_, err = f.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for ; number <= limit; number++ {
			...
			// zena block receipt
			zenaBlockReceipt := ReadZenaReceiptRLP(nfdb, hash, number)
			if err := op.AppendRaw(freezerZenaReceiptTable, number, zenaBlockReceipt); err != nil {
				return fmt.Errorf("can't write zena-receipt to freezer: %v", err)
			}
