...
func WriteAncientBlocks(db ethdb.AncientWriter, blocks []*types.Block, receipts []types.Receipts, zenaReceipts []types.Receipts, td *big.Int) (int64, error) {
	var (
		tdSum          = new(big.Int).Set(td)
		stReceipts     []*types.ReceiptForStorage
		zenaStReceipts []*types.ReceiptForStorage
	)
	...
	return db.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for i, block := range blocks {
			...
			// Convert zena receipts to storage format and sum up total difficulty.
			zenaStReceipts = zenaStReceipts[:0]
			for _, zenaReceipt := range zenaReceipts[i] {
				zenaStReceipts = append(zenaStReceipts, (*types.ReceiptForStorage)(zenaReceipt))
			}
			...
			if err := writeAncientBlock(op, block, header, stReceipts, zenaStReceipts, tdSum); err != nil {
				return err
			}
		}
...
func writeAncientBlock(op ethdb.AncientWriteOp, block *types.Block, header *types.Header, receipts []*types.ReceiptForStorage, zenaReceipts []*types.ReceiptForStorage, td *big.Int) error {
	num := block.NumberU64()
	...
	if err := op.Append(freezerZenaReceiptTable, num, zenaReceipts); err != nil {
		return fmt.Errorf("can't append block %d zenaReceipts: %v", num, err)
	}
...
func DeleteBlock(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
	...
	// delete zena receipt
	DeleteZenaReceipt(db, hash, number)
}
