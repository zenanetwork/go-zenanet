...
func init() {
	...
	/*
		Given the introduction of `ZenaDefaultTxPoolPriceLimit=25gwei`,
		we set `testTxPoolConfig.PriceLimit = 1` to avoid rewriting all `legacypool_test.go` tests,
		causing code divergence from gzen, as this has been widely tested on different networks.
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(uint256.Int).SetUint64(params.Zen), tracing.BalanceChangeUnspecified)
		*c.trigger = false
	}
...
// TestTxPoolDefaultPriceLimit ensures the zena default tx pool price limit is set correctly.
func TestTxPoolDefaultPriceLimit(t *testing.T) {
	...
	if have, want := pool.config.PriceLimit, uint64(params.ZenaDefaultTxPoolPriceLimit); have != want {
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(uint256.Int).SetUint64(params.Zen), tracing.BalanceChangeUnspecified)
