...
func TestSetCodeTransactions(t *testing.T) {
	...
	testAddBalance(pool, addrA, big.NewInt(params.Zen))
	testAddBalance(pool, addrB, big.NewInt(params.Zen))
	testAddBalance(pool, addrC, big.NewInt(params.Zen))
		...
		},
		{
			...
			run: func(name string) {
				...
				for i := 0; i < 30; i++ {
					...
					addr := crypto.PubkeyToAddress(key.PublicKey)
					testAddBalance(pool, addr, big.NewInt(params.Zen))
				}
