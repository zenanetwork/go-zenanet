...
// Package core implements the Zenanet consensus protocol.
package core
...
import (
	...
	"github.com/zenanetwork/go-zenanet/core/state/snapshot"
	...
	"github.com/zenanetwork/go-zenanet/internal/syncx"
...
var (
	...
	zenaConsensusTime = metrics.NewRegisteredTimer("chain/zena/consensus", nil)
...
type CacheConfig struct {
	...
	StateScheme         string        // Scheme used to store zenanet states and merkle tree nodes on top
...
type BlockChain struct {
	...
	// Zena related changes
	zenaReceiptsCache *lru.Cache[common.Hash, *types.Receipt] // Cache for the most recent zena receipt receipts per block
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64, checker zenanet.ChainValidator) (*BlockChain, error) {
	...
	bc := &BlockChain{
		...
		zenaReceiptsCache: lru.NewCache[common.Hash, *types.Receipt](receiptsCacheLimit),
	...
	// If Gzen is initialized with an external ancient store, re-initialize the
	...
	// it in advance.
	// ZENA - commented out intentionally
...
func NewParallelBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64, checker zenanet.ChainValidator, numprocs int, enforce bool) (*BlockChain, error) {
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	delFn := func(db ethdb.KeyValueWriter, hash common.Hash, num uint64) {
		...
		if num+1 <= frozen {
			...
			rawdb.DeleteZenaReceipt(db, hash, num)
			rawdb.DeleteZenaTxLookupEntry(db, hash, num)
		}
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		// ZENA: Retrieve all the zena receipts and also maintain the array of headers
		// for zena specific reorg check.
		zenaReceipts := []types.Receipts{}

		var headers []*types.Header

		for _, block := range blockChain {
			zenaReceipts = append(zenaReceipts, []*types.Receipt{bc.GetZenaReceiptByHash(block.Hash())})
			headers = append(headers, block.Header())
		}

		// Write all chain data to ancients.
		td := bc.GetTd(first.Hash(), first.NumberU64())
		writeSize, err := rawdb.WriteAncientBlocks(bc.db, blockChain, receiptChain, zenaReceipts, td)
		if err != nil {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Gzen is initialized with
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			// Update the headers for zena specific reorg check
			headers = append(headers, block.Header())
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, statedb *state.StateDB) ([]*types.Log, error) {
	...
	if len(blockLogs) > 0 {
		...
		if len(blockLogs) > len(logs) {
			...
			// DeriveFieldsForZenaLogs will fill those fields for websocket subscriptions
			types.DeriveFieldsForZenaLogs(stateSyncLogs, block.Hash(), block.NumberU64(), uint(len(receipts)), uint(len(logs)))

			// Write zena receipt
			rawdb.WriteZenaReceipt(blockBatch, block.Hash(), block.NumberU64(), &types.ReceiptForStorage{
				Status: types.ReceiptStatusSuccessful, // make receipt status successful
				Logs:   stateSyncLogs,
			})

			// Write zena tx reverse lookup
			rawdb.WriteZenaTxLookupEntry(blockBatch, block.Hash(), block.NumberU64())
		}
	}
...
func (bc *BlockChain) writeBlockAndSetHead(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	if status == CanonStatTy {
		...
		if emitHeadEvent {
			bc.chainHeadFeed.Send(ChainHeadEvent{Block: block})
			// ZENA state sync feed related changes
...
// it's not checked in the Gzen side anymore.
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		// ZENA
		ptime := time.Since(pstart) - vtime - statedb.ZenaConsensusTime
		...
		zenaConsensusTime.Update(statedb.ZenaConsensusTime)             // The time spent on zena consensus (span + state sync)
...
func (bc *BlockChain) collectLogs(b *types.Block, removed bool) []*types.Log {
	...
	// Append zena receipt
	zenaReceipt := rawdb.ReadZenaReceipt(bc.db, b.Hash(), b.NumberU64(), bc.chainConfig)
	if zenaReceipt != nil {
