...
func TestGeneratePOSChain(t *testing.T) {
	var (
		...
		funds   = big.NewInt(0).Mul(big.NewInt(1337), big.NewInt(params.Zen))
...
func ExampleGenerateChain() {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), genDb, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			// In block 1, addr1 sends addr2 some zen.
			tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr1), addr2, big.NewInt(10000), params.TxGas, nil, nil), signer, key1)
			gen.AddTx(tx)
		case 1:
			// In block 2, addr1 sends some more zen to addr2.
