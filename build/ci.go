...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	...
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/zenanetwork/go-zenanet/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/zenanetwork/go-zenanet/internal/version.gitDate="+env.Date)
	}
...
func downloadSpecTestFixtures(csdb *build.ChecksumDB, cachedir string) string {
	...
	url := fmt.Sprintf("https://github.com/zenanetwork/execution-spec-tests/releases/download/%s/%s%s", executionSpecTestsVersion, base, ext)
...
func doCheckBadDeps() {
	baddeps := [][2]string{
		...
		{"github.com/zenanetwork/go-zenanet/core/rawdb", "github.com/zenanetwork/go-zenanet/ethdb/leveldb"},
		{"github.com/zenanetwork/go-zenanet/core/rawdb", "github.com/zenanetwork/go-zenanet/ethdb/pebbledb"},
	}
	tc := new(build.GoToolchain)
...
func doArchive(cmdline []string) {
	...
	var (
		env      = build.Env()
		basegzen = archiveBasename(*arch, version.Archive(env.Commit))
...
func doDockerBuildx(cmdline []string) {
	var (
		...
		hubImage = flag.String("hub", "zenanet/client-go", `Where to upload the docker image`)
	...
	for _, spec := range []struct {
		...
		for _, tag := range tags { // latest, stable etc
			gzenImage := fmt.Sprintf("%s%s", spec.base, tag)
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	installer, err := filepath.Abs("gzen-" + archiveBasename(*arch, version.Archive(env.Commit)) + ".exe")
	if err != nil {
