...
import (
	...
	"github.com/zenanetwork/go-zenanet/crypto/signify"
	"github.com/zenanetwork/go-zenanet/internal/build"
	"github.com/zenanetwork/go-zenanet/params"
)

var (
	// Files that end up in the gzen*.zip archive.
	gzenArchiveFiles = []string{
		"COPYING",
		executablePath("gzen"),
	}

	// Files that end up in the gzen-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Zenanet contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Zenanet bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Zenanet Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gzen",
			Description: "Zenanet CLI client.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Zenanet account management tool.",
		},
	}

	// A debian package is created for all executables listed here.
	debZenanet = debPackage{
		Name:        "zenanet",
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/maticnetwork/zena/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/maticnetwork/zena/internal/version.gitDate="+env.Date)
	}
...
func downloadSpecTestFixtures(csdb *build.ChecksumDB, cachedir string) string {
	...
	url := fmt.Sprintf("https://github.com/zenanetwork/execution-spec-tests/releases/download/v%s/%s%s", executionSpecTestsVersion, base, ext)
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "gzenstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		basegzen = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		gzen     = "gzen-" + basegzen + ext
		alltools = "gzen-alltools-" + basegzen + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gzen, gzenArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gzen, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gzen-release.pub"
...
func doDocker(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "zenanet/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - zenanet/client-go:latest                            - Pushes to the master branch, Gzen only
	//  - zenanet/client-go:stable                            - Version tag publish on GitHub, Gzen only
	//  - zenanet/client-go:alltools-latest                   - Pushes to the master branch, Gzen & tools
	//  - zenanet/client-go:alltools-stable                   - Version tag publish on GitHub, Gzen & tools
	//  - zenanet/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Gzen only
	//  - zenanet/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Gzen & tools
	//  - zenanet/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Gzen only
	//  - zenanet/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Gzen & tools
	var tags []string
	...
	if *image {
		...
		for _, tag := range tags {
			gzenImage := fmt.Sprintf("%s:%s-%s", *upload, tag, runtime.GOARCH)
			...
			// master branch.
			for _, img := range []string{gzenImage, toolImage} {
			...
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:TAG", *upload), gzenImage)
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:alltools-TAG", *upload), toolImage)
			build.MustRunCommand("docker", "push", gzenImage)
	...
	if len(*manifest) != 0 {
		...
		for i := 0; i < 2; i++ { // 2 attempts, second is race check
			...
			for _, tag := range tags {
				for _, arch := range strings.Split(*manifest, ",") {
					gzenImage := fmt.Sprintf("%s:%s-%s", *upload, tag, arch)
		...
		// Assemble and push the Gzen manifest image
		for _, tag := range tags {
			gzenImage := fmt.Sprintf("%s:%s", *upload, tag)

			var gzenSubImages []string
			for _, arch := range strings.Split(*manifest, ",") {
				gzenSubImages = append(gzenSubImages, gzenImage+"-"+arch)
			}
			build.MustRunCommand("docker", append([]string{"manifest", "create", gzenImage}, gzenSubImages...)...)
			build.MustRunCommand("docker", "manifest", "push", gzenImage)
		}
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "zenanet/zenanet")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gzen-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for _, distro := range debDistros {
			// Prepare the debian package with the go-zenanet sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gzen-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "zenanet"
...
type debMetadata struct {
	...
	// go-zenanet version being built. Note that this
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Zenanet Builds <fjl@ethereum.org>"
	}
	return debMetadata{
		PackageName: name,
		Env:         env,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gzen.exe" {
			gzenTool = file
		} else {
	...
	// first section contains the gzen binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gzen":     gzenTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gzen.nsi", filepath.Join(*workdir, "gzen.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("gzen-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gzen.nsi"),
	)
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gzenstore/builds")`)
