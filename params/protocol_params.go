...
const (
	...
	MaxCodeSizePostAhmedabad = 32768           // Maximum bytecode to permit for a contract post Ahmedabad hard fork (zena / zena pos) (32KB)
	...
	// ZenaDefaultMinerGasPrice defines the minimum gas price for zena validators to mine a transaction.
	ZenaDefaultMinerGasPrice = 25 * GWei

	// ZenaDefaultTxPoolPriceLimit defines the minimum gas price limit for zena to enforce txs acceptance into the pool.
	ZenaDefaultTxPoolPriceLimit = 25 * GWei

	// ZenaDefaultGpoIgnorePrice defines the minimum gas price below which zena gpo will ignore transactions.
	ZenaDefaultGpoIgnorePrice = 25 * GWei
)
...
func BaseFeeChangeDenominator(zenaConfig *ZenaConfig, number *big.Int) uint64 {
	if zenaConfig.IsDelhi(number) {
