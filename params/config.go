...
var (
	...
	// ZenaTestChainConfig contains the chain parameters to run a node on the Test network.
	ZenaTestChainConfig = &ChainConfig{
	...
	ZenaUnittestChainConfig = &ChainConfig{
	...
	// EireneChainConfig contains the chain parameters to run a node on the Eirene test network.
	EireneChainConfig = &ChainConfig{
	...
	// PaxChainConfig contains the chain parameters to run a node on the Pax test network.
	PaxChainConfig = &ChainConfig{
		ChainID:             big.NewInt(80002),
		HomesteadBlock:      big.NewInt(0),
		DAOForkBlock:        nil,
		DAOForkSupport:      true,
...
var NetworkNames = map[string]string{
	...
	EireneChainConfig.ChainID.String():  "eirene",
	PaxChainConfig.ChainID.String():     "pax",
}
...
type ChainConfig struct {
	...
	// Fork scheduling switched back to blockNumber in Zena
	...
	Zena      *ZenaConfig   `json:"zena,omitempty"`
...
func (c *ZenaConfig) CalculateProducerDelay(number uint64) uint64 {
	return zenaKeyValueConfigHelper(c.ProducerDelay, number)
}

func (c *ZenaConfig) CalculateSprint(number uint64) uint64 {
	return zenaKeyValueConfigHelper(c.Sprint, number)
}

func (c *ZenaConfig) CalculateBackupMultiplier(number uint64) uint64 {
	return zenaKeyValueConfigHelper(c.BackupMultiplier, number)
}

func (c *ZenaConfig) CalculatePeriod(number uint64) uint64 {
	return zenaKeyValueConfigHelper(c.Period, number)
}

// // TODO: modify this function once the block number is finalized
// func (c *ZenaConfig) IsNapoli(number *big.Int) bool {
...
func (c *ZenaConfig) IsSprintStart(number uint64) bool {
	return number%c.CalculateSprint(number) == 0
}

func zenaKeyValueConfigHelper[T uint64 | string](field map[string]T, number uint64) T {
...
func (c *ChainConfig) Description() string {
	...
	if c.TerminalTotalDifficulty == nil {
		banner += "The Merge is not available for this network\n"
