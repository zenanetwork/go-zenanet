...
import (
	...
	"github.com/zenanetwork/go-zenanet/p2p/msgrate"
...
type accountTask struct {
	...
	// this additional field will be ignored by legacy Gzen. The only side
...
// Syncer is an Zenanet account and storage trie syncer based on snapshots and
...
// NewSyncer creates a new snapshot syncer to download the Zenanet state over the
// snap protocol.
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	// no longer possible in zenanet as self-destruction is disabled in Cancun
	...
	if len(res.hashes) > 0 {
		...
		for hash := range res.task.SubTasks {
			...
			// TODO(rjl493456442) degrade the log level before merging.
			// It should never happen in zenanet.
