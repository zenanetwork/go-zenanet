...
// testBackend is a mock implementation of the live Zenanet message handler. Its
...
func newTestBackendWithGenerator(blocks int, shanghai bool, generator func(int, *core.BlockGen)) *testBackend {
	...
	if shanghai {
		config = &params.ChainConfig{
			...
			Zena:                          params.TestChainConfig.Zena,
		}
...
func testGetBlockReceipts(t *testing.T, protocol uint) {
	...
	generator := func(i int, block *core.BlockGen) {
		switch i {
		case 0:
			// In block 1, the test bank sends account #1 some zen.
			...
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more zen to account #1.
