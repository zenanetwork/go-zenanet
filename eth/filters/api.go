...
// information related to the Zenanet protocol such as blocks, transactions and logs.
...
func NewFilterAPI(system *FilterSystem, zenaLogs bool) *FilterAPI {
	api := &FilterAPI{
		sys:      system,
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(zenanet.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as zenanet.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria zenanet.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(zenanet.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
...
func (api *FilterAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	zenaConfig := api.sys.backend.ChainConfig().Zena

	var filter *Filter

	var zenaLogsFilter *ZenaBlockLogsFilter
	...
	if crit.BlockHash != nil {
		...
		// Block zena filter
		if api.zenaLogs {
			zenaLogsFilter = NewZenaBlockLogsFilter(api.sys.backend, zenaConfig, *crit.BlockHash, crit.Addresses, crit.Topics)
		}
	} else {
		...
		// Block zena filter
		if api.zenaLogs {
			zenaLogsFilter = NewZenaBlockLogsRangeFilter(api.sys.backend, zenaConfig, begin, end, crit.Addresses, crit.Topics)
		}
	}
	...
	if zenaLogsFilter != nil {
		// Run the filter and return all the logs
		zenaBlockLogs, err := zenaLogsFilter.Logs(ctx)
		if err != nil {
			return nil, err
		}

		return returnLogs(types.MergeZenaLogs(logs, zenaBlockLogs)), err
	}

	// merge zena block logs and receipt logs and return it
...
func (api *FilterAPI) GetFilterLogs(ctx context.Context, id rpc.ID) ([]*types.Log, error) {
	...
	if f.crit.BlockHash != nil {
		...
		// Block zena filter
		if api.zenaLogs {
			zenaLogsFilter = NewZenaBlockLogsFilter(api.sys.backend, zenaConfig, *f.crit.BlockHash, f.crit.Addresses, f.crit.Topics)
		}
	} else {
		// Convert the RPC block numbers into internal representations
		begin := rpc.LatestBlockNumber.Int64()
		...
		if api.zenaLogs {
			zenaLogsFilter = NewZenaBlockLogsRangeFilter(api.sys.backend, zenaConfig, begin, end, f.crit.Addresses, f.crit.Topics)
		}
	}
	// Run the filter and return all the logs
	logs, err := filter.Logs(ctx)
	...
	if zenaLogsFilter != nil {
		...
		return returnLogs(types.MergeZenaLogs(logs, zenaBlockLogs)), nil
	}
