...
func (b *testBackend) GetZenaBlockReceipt(ctx context.Context, blockHash common.Hash) (*types.Receipt, error) {
	number := rawdb.ReadHeaderNumber(b.db, blockHash)
	if number == nil {
		return &types.Receipt{}, nil
	}

	receipt := rawdb.ReadZenaReceipt(b.db, blockHash, *number, nil)
	if receipt == nil {
		return &types.Receipt{}, nil
	}
...
func (b *testBackend) GetZenaBlockLogs(ctx context.Context, blockHash common.Hash) ([]*types.Log, error) {
	receipt, err := b.GetZenaBlockReceipt(ctx, blockHash)
	if err != nil {
...
func TestInvalidGetLogsRequest(t *testing.T) {
	...
	api.SetChainConfig(params.ZenaUnittestChainConfig)
...
func TestInvalidGetRangeLogsRequest(t *testing.T) {
	...
	api.SetChainConfig(params.ZenaTestChainConfig)
