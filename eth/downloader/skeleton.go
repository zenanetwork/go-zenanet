...
// the process. This is usually the case when Gzen is shutting down and some events
...
//
// Opposed to the original Zenanet block synchronization which is trustless (and
...
// not from within (vs. legacy Zenanet sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gzen is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gzen requests a termination.
					errc := <-s.terminate
					errc <- err
