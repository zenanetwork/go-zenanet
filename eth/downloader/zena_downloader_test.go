...
func testSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", zenanet.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", zenanet.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", zenanet.SyncProgress{
	...
	pending.Wait()
	checkProgress(t, tester.downloader, "final", zenanet.SyncProgress{
		StartingBlock: uint64(len(chain.blocks)/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want zenanet.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", zenanet.SyncProgress{
