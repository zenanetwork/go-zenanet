...
func newZenaVerifier() *zenaVerifier {
	return &zenaVerifier{zenaVerify}
}

func zenaVerify(ctx context.Context, eth *Zenanet, handler *ethHandler, start uint64, end uint64, hash string, isCheckpoint bool) (string, error) {
	str := "milestone"
	if isCheckpoint {
		str = "checkpoint"
	}
...
func rewindBack(eth *Zenanet, head uint64, rewindTo uint64) {
	if eth.Miner().Mining() {
		ch := make(chan struct{})
		eth.Miner().Stop(ch)
...
func rewind(eth *Zenanet, head uint64, rewindTo uint64) {
