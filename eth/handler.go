...
import (
	...
	"github.com/zenanetwork/go-zenanet/core/forkid"
	...
	"github.com/zenanetwork/go-zenanet/eth/fetcher"
...
// support all the operations needed by the Zenanet chain protocols.
...
type handlerConfig struct {
	...
	checker             zenanet.ChainValidator
...
// newHandler returns a handler for all Zenanet chain management protocol.
func newHandler(config *handlerConfig) (*handler, error) {
	...
	if config.Sync == downloader.FullSync {
		...
		// TODO - uncomment when we (Zena-PoS, zena) have snap sync/pbss
		// fullBlock, snapBlock := h.chain.CurrentBlock(), h.chain.CurrentSnapBlock()

		// TODO - uncomment when we (Zena-PoS, zena) have snap sync/pbss
		// For more info - https://github.com/zenanetwork/go-zenanet/pull/28171
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Zenanet handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Zenanet peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Zenanet peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Zenanet peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Zenanet peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Zenanet peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Zenanet protocol stopped")
}
