...
func (api *API) traceZenaBlock(ctx context.Context, block *types.Block, config *TraceConfig) (*BlockTraceResult, error) {
	...
	traceTxn := func(indx int, tx *types.Transaction, zenaTx bool, stateSyncHash common.Hash) *TxTraceResult {
		...
		if zenaTx {
			...
			execRes, err = statefull.ApplyZenaMessage(vmenv, callmsg)
		} else {
...
func (api *API) TraceZenaBlock(req *TraceBlockRequest) (*BlockTraceResult, error) {
	...
	log.Debug("Tracing Zena Block", "block number", blockNumber)
	...
	return api.traceZenaBlock(ctx, block, req.Config)
}
