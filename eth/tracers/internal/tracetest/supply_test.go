...
func TestSupplyGenesisAlloc(t *testing.T) {
	var (
		...
		eth1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Zen))
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			GenesisAlloc: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Zen))),
		},
		Number:     0,
...
func TestSupplyRewards(t *testing.T) {
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			Reward: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Zen))),
		},
		Number:     1,
...
func TestSupplyEip1559Burn(t *testing.T) {
	...
	var (
		...
		reward   = new(big.Int).Mul(common.Big2, big.NewInt(params.Zen))
...
// the zen sent in between is burnt before Cancun hard fork.
...
func TestSupplySelfdestructItselfAndRevert(t *testing.T) {
	var (
		...
		gwei5   = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.GWei))
		eth1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Zen))
		eth2    = new(big.Int).Mul(common.Big2, big.NewInt(params.Zen))
		eth5    = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.Zen))
	...
	// Check balance at state:
	// 1. A has 0 zen
	// 2. B has 0 zen, burned
	// 3. C has 2 zen, selfdestructed but parent D reverted
	// 4. D has 1 zen, reverted
