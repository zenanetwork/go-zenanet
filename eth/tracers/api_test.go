	...
	var testSuite = []struct {
		...
		// Upon the last state, default to the post block's state
		{
			...
			call: ethapi.TransactionArgs{
				...
				To:    &accounts[0].addr,
				Value: (*hexutil.Big)(new(big.Int).Add(big.NewInt(params.Zen), big.NewInt(100))),
			},
			config: nil,
			expect: `{"gas":21000,"failed":false,"returnValue":"","structLogs":[]}`,
		},
...
func TestIOdump(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: types.GenesisAlloc{
		...
		accounts[1].addr: {Balance: big.NewInt(params.Zen)},
		accounts[2].addr: {Balance: big.NewInt(params.Zen)},
		accounts[3].addr: {Balance: big.NewInt(params.Zen)},
		accounts[4].addr: {Balance: big.NewInt(params.Zen)},
	}}
	genBlocks := 1
	signer := types.HomesteadSigner{}
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &ethapi.StateOverride{
					randomAccounts[0].addr: ethapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Zen)))},
				},
			},
...
func TestTraceBlockWithBasefee(t *testing.T) {
	...
	genesis := &core.Genesis{
		...
		Alloc: types.GenesisAlloc{
			accounts[0].addr: {Balance: big.NewInt(1 * params.Zen)},
