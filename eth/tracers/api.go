...
var defaultZenaTraceEnabled = newBoolPtr(false)
...
// NewAPI creates a new API definition for the tracing methods of the Zenanet service.
...
func (api *API) getAllBlockTransactions(ctx context.Context, block *types.Block) (types.Transactions, bool, common.Hash) {
	...
	zenaReceipt := rawdb.ReadZenaReceipt(api.backend.ChainDb(), block.Hash(), block.NumberU64(), api.backend.ChainConfig())
	if zenaReceipt != nil {
		txHash := types.GetDerivedZenaTxHash(types.ZenaReceiptKey(block.Number().Uint64(), block.Hash()))
		if txHash != (common.Hash{}) {
			zenaTx, _, _, _, _ := api.backend.GetZenaBlockTransactionWithBlockHash(ctx, txHash, block.Hash())
...
func (api *API) traceChain(start, end *types.Block, config *TraceConfig, closed <-chan error) chan *blockTraceResult {
	if config == nil {
		config = &TraceConfig{
			ZenaTraceEnabled: defaultZenaTraceEnabled,
			ZenaTx:           newBoolPtr(false),
		}
	}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range taskCh {
				...
				if !*config.ZenaTraceEnabled && stateSyncPresent {
					txs = txs[:len(txs)-1]
					stateSyncPresent = false
				}
				...
				for i, tx := range txs {
					...
					if stateSyncPresent && i == len(txs)-1 {
						if *config.ZenaTraceEnabled {
							config.ZenaTx = newBoolPtr(true)
						}
					}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				// concurrent use.
				// See: https://github.com/zenanetwork/go-zenanet/issues/29114
		...
		if !ioflag {
			...
			if stateSyncPresent && i == len(txs)-1 {
				if *config.ZenaTraceEnabled {
					callmsg := prepareCallMessage(*msg)
					// nolint : contextcheck
					if _, err := statefull.ApplyZenaMessage(vmenv, callmsg); err != nil {
						failed = err
						break txloop
					}
				} else {
	...
	for i, tx := range block.Transactions() {
		...
		if stateSyncPresent && i == len(txs)-1 {
			if *config.ZenaTraceEnabled {
				callmsg := prepareCallMessage(*msg)
				statedb.SetTxContext(stateSyncHash, i)
				_, err = statefull.ApplyZenaMessage(vmenv, callmsg)
...
func (api *API) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	if !found {
		// For ZenaTransaction, there will be no trace available
		tx, _, _, _ := rawdb.ReadZenaTransaction(api.backend.ChainDb(), hash)
		if tx != nil {
...
func (api *API) traceTx(ctx context.Context, tx *types.Transaction, message *core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	if config.ZenaTx == nil {
		config.ZenaTx = newBoolPtr(false)
	}
