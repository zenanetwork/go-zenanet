...
func TestZenaFilters(t *testing.T) {
	...
	testZenaConfig.Sprint = map[string]uint64{
		"0": 4,
		"8": 2,
	}
	...
	for i, block := range chain {
		...
		if len(receipts[i]) > 0 {
			...
			// State sync logs don't have tx index, tx hash and other necessary fields, DeriveFieldsForZenaLogs will fill those fields for websocket subscriptions
			// DeriveFieldsForZenaLogs argurments:
			// 1. State-sync logs
			// 2. Block Hash
			// 3. Block Number
			...
			types.DeriveFieldsForZenaLogs(receipts[i][0].Logs, block.Hash(), block.NumberU64(), uint(0), uint(0))
	...
	filter := filters.NewZenaBlockLogsRangeFilter(backend, testZenaConfig, 0, -1, []common.Address{addr}, [][]common.Hash{{hash1, hash2, hash3, hash4, hash5}})
	...
	filter = filters.NewZenaBlockLogsRangeFilter(backend, testZenaConfig, 900, 999, []common.Address{addr}, [][]common.Hash{{hash3}})
	logs, _ = filter.Logs(context.Background())
	...
	filter = filters.NewZenaBlockLogsRangeFilter(backend, testZenaConfig, 992, -1, []common.Address{addr}, [][]common.Hash{{hash3}})
	logs, _ = filter.Logs(context.Background())
	...
	filter = filters.NewZenaBlockLogsRangeFilter(backend, testZenaConfig, 1, -1, []common.Address{addr}, [][]common.Hash{{hash1, hash2}})
	...
	filter = filters.NewZenaBlockLogsRangeFilter(backend, testZenaConfig, 0, -1, nil, [][]common.Hash{{failHash}})
	...
	filter = filters.NewZenaBlockLogsRangeFilter(backend, testZenaConfig, 0, -1, []common.Address{failAddr}, nil)
	...
	filter = filters.NewZenaBlockLogsRangeFilter(backend, testZenaConfig, 0, -1, nil, [][]common.Hash{{failHash}, {hash1}})
