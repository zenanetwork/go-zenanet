...
func testGetTransactionReceiptsByBlock(t *testing.T, publicBlockchainAPI *ethapi.BlockChainAPI) {
	...
	txHash := types.GetDerivedZenaTxHash(types.ZenaReceiptKey(4, blockHash))
...
func TestAPIs(t *testing.T) {
	...
	for i, block := range chain {
		...
		if i%int(testZenaConfig.CalculateSprint(block.NumberU64())-1) != 0 {
			...
			if len(receipts[i]) > 0 {
				...
				types.DeriveFieldsForZenaLogs(receipts[i][1].Logs, block.Hash(), block.NumberU64(), uint(1), uint(1))
