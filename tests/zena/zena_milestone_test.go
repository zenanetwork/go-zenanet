...
func TestRewinding(t *testing.T) {
	...
	for {
		...
		if blockHeaderVal0.Number.Uint64() == 15 && !step1 {
			_, _ = zenaVerify(nodes[0], 8, 15, "LocalHash", "RootHash", 15, 7)
			step1 = true
		}
		...
		if blockHeaderVal0.Number.Uint64() == 270 && !step2 {
			_, _ = zenaVerify(nodes[0], 8, 15, "LocalHash", "RootHash", 270, 7)
			step2 = true
		}
...
func zenaVerify(eth *eth.Zenanet, start uint64, end uint64, rootHash string, localHash string, head uint64, lastMilestone uint64) (string, error) {
	//nolint
...
func rewindBack(eth *eth.Zenanet, rewindTo uint64) {
	if eth.Miner().Mining() {
		ch := make(chan struct{})
		eth.Miner().Stop(ch)
		<-ch
...
func rewind(eth *eth.Zenanet, rewindTo uint64) {
