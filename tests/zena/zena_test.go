...
func TestForkWithBlockTime(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			genesis := InitGenesis(t, faucets, "./testdata/genesis_2val.json", 8)
			genesis.Config.Zena.Sprint = test.sprint
			genesis.Config.Zena.Period = test.blockTime
			genesis.Config.Zena.BackupMultiplier = test.blockTime
			genesis.Config.Zena.ProducerDelay = test.producerDelay
...
func TestInsertingSpanSizeBlocks(t *testing.T) {
	...
	init := buildZenanetInstance(t, rawdb.NewMemoryDatabase())
	chain := init.zenanet.BlockChain()
	engine := init.zenanet.Engine()
	_zena := engine.(*zena.Zena)

	_, currentSpan := loadSpanFromFile(t)

	h, ctrl := getMockedIrisClient(t, currentSpan)
	defer func() {
		_zena.Close()
		ctrl.Finish()
	}()
	...
	_zena.SetSpanner(spanner)

	// Insert sprintSize # of blocks so that span is fetched at the start of a new sprint
	for i := uint64(1); i <= spanSize; i++ {
		block = buildNextBlock(t, _zena, chain, block, nil, init.genesis.Config.Zena, nil, currentValidators)
	...
	validators, err := _zena.GetCurrentValidators(context.Background(), block.Hash(), spanSize) // check validator set at the first block of new span
	if err != nil {
		t.Fatalf("%s", err)
	}
...
func TestFetchStateSyncEvents(t *testing.T) {
	...
	// B. Before inserting 1st block of the next sprint, mock iris deps
	...
	block = buildNextBlock(t, _zena, chain, block, nil, init.genesis.Config.Zena, nil, res.Result.ValidatorSet.Validators)
...
func TestFetchStateSyncEvents_2(t *testing.T) {
	...
	lastStateID, _ := _zena.GenesisContractsClient.LastStateId(nil, sprintSize, block.Hash())
	...
	lastStateID, _ = _zena.GenesisContractsClient.LastStateId(nil, spanSize, block.Hash())
...
func TestOutOfTurnSigning(t *testing.T) {
	...
	_, irisSpan := loadSpanFromFile(t)
	proposer := valset.NewValidator(addr, 10)
	irisSpan.ValidatorSet.Validators = append(irisSpan.ValidatorSet.Validators, proposer)

	// add the block producer
	h, ctrl := getMockedIrisClient(t, irisSpan)
	defer ctrl.Finish()
	...
	spanner := getMockedSpanner(t, irisSpan.ValidatorSet.Validators)
	_zena.SetSpanner(spanner)
	...
	setDifficulty := func(header *types.Header) {
		if IsSprintStart(header.Number.Uint64()) {
			header.Difficulty = big.NewInt(int64(len(irisSpan.ValidatorSet.Validators)))
		}
	}

	for i := uint64(1); i < spanSize; i++ {
		block = buildNextBlock(t, _zena, chain, block, nil, init.genesis.Config.Zena, nil, irisSpan.ValidatorSet.Validators, setDifficulty)
		insertNewBlock(t, chain, block)
	}
	...
	setDifficulty = func(header *types.Header) {
		header.Difficulty = big.NewInt(int64(len(irisSpan.ValidatorSet.Validators)) - turn)
	}

	block = buildNextBlock(t, _zena, chain, block, signerKey, init.genesis.Config.Zena, nil, irisSpan.ValidatorSet.Validators, setParentTime, setDifficulty)
	_, err := chain.InsertChain([]*types.Block{block})
	require.Equal(t,
		zena.BlockTooSoonError{Number: spanSize, Succession: expectedSuccessionNumber},
		*err.(*zena.BlockTooSoonError))

	expectedDifficulty := uint64(len(irisSpan.ValidatorSet.Validators) - expectedSuccessionNumber - turn) // len(validators) - succession
	header := block.Header()

	diff := zena.CalcProducerDelay(header.Number.Uint64(), expectedSuccessionNumber, init.genesis.Config.Zena)
	header.Time += diff

	sign(t, header, signerKey, init.genesis.Config.Zena)
	...
	require.Equal(t,
		zena.WrongDifficultyError{Number: spanSize, Expected: expectedDifficulty, Actual: 3, Signer: newAddr.Bytes()},
		*err.(*zena.WrongDifficultyError))
...
func TestSignerNotFound(t *testing.T) {
	...
	_zena.Authorize(newAddr, func(account accounts.Account, s string, data []byte) ([]byte, error) {
		return crypto.Sign(crypto.Keccak256(data), newKey)
	})

	block = buildNextBlock(t, _zena, chain, block, signerKey, init.genesis.Config.Zena, nil, irisSpan.ValidatorSet.Validators)

	_, err := chain.InsertChain([]*types.Block{block})
	require.Equal(t,
		*err.(*zena.UnauthorizedSignerError),
		zena.UnauthorizedSignerError{Number: 0, Signer: newAddr.Bytes()})
}
...
func TestEIP1559Transition(t *testing.T) {
	var (
		...
		funds = new(big.Int).Mul(common.Big1, big.NewInt(params.Zen))
		gspec = &core.Genesis{
			Config: params.ZenaUnittestChainConfig,
	...
	actual = state.GetBalance(common.HexToAddress(params.ZenaUnittestChainConfig.Zena.CalculateBurntContract(block.NumberU64()))).ToBig()
...
func TestBurnContract(t *testing.T) {
	...
	gspec.Config.BerlinBlock = common.Big0
	gspec.Config.LondonBlock = common.Big0
	gspec.Config.Zena.BurntContract = map[string]string{
	...
	// check burnt contract balance
	actual = state.GetBalance(common.HexToAddress(gspec.Config.Zena.CalculateBurntContract(block.NumberU64()))).ToBig()
	expected = new(big.Int).Mul(new(big.Int).SetUint64(block.GasUsed()), block.BaseFee())
	if actual.Cmp(expected) != 0 {
		t.Fatalf("burnt contract balance incorrect: expected %d, got %d", expected, actual)
	}
...
func TestBurnContractContractFetch(t *testing.T) {
	t.Parallel()
	config := params.ZenaUnittestChainConfig
	config.Zena.BurntContract = map[string]string{
		"10":  "0x000000000000000000000000000000000000aaab",
		"100": "0x000000000000000000000000000000000000aaad",
	}

	burnContractAddr10 := config.Zena.CalculateBurntContract(10)
	burnContractAddr11 := config.Zena.CalculateBurntContract(11)
	burnContractAddr99 := config.Zena.CalculateBurntContract(99)
	burnContractAddr100 := config.Zena.CalculateBurntContract(100)
	burnContractAddr101 := config.Zena.CalculateBurntContract(101)
	...
	burnContractAddr100 = config.Zena.CalculateBurntContract(100)
	burnContractAddr101 = config.Zena.CalculateBurntContract(101)
	burnContractAddr999 := config.Zena.CalculateBurntContract(999)
	burnContractAddr1000 := config.Zena.CalculateBurntContract(1000)
	burnContractAddr1001 := config.Zena.CalculateBurntContract(1001)
...
func TestJaipurFork(t *testing.T) {
	...
	for i := uint64(1); i < sprintSize; i++ {
		...
		if block.Number().Uint64() == init.genesis.Config.Zena.JaipurBlock.Uint64()-1 {
			require.Equal(t, testSealHash(block.Header(), init.genesis.Config.Zena), zena.SealHash(block.Header(), init.genesis.Config.Zena))
		}

		if block.Number().Uint64() == init.genesis.Config.Zena.JaipurBlock.Uint64() {
			require.Equal(t, testSealHash(block.Header(), init.genesis.Config.Zena), zena.SealHash(block.Header(), init.genesis.Config.Zena))
		}
	}
}
...
func testSealHash(header *types.Header, c *params.ZenaConfig) (hash common.Hash) {
...
func testEncodeSigHeader(w io.Writer, header *types.Header, c *params.ZenaConfig) {
	enc := []interface{}{
		header.ParentHash,
		header.UncleHash,
		header.Coinbase,
