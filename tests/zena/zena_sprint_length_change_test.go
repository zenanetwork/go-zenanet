...
func TestSprintLengths(t *testing.T) {
	...
	assert.Equal(t, testZenaConfig.CalculateSprint(0), uint64(16))
	assert.Equal(t, testZenaConfig.CalculateSprint(8), uint64(4))
	assert.Equal(t, testZenaConfig.CalculateSprint(9), uint64(4))
}
...
func TestProducerDelay(t *testing.T) {
	...
	testZenaConfig := params.TestChainConfig.Zena
	testZenaConfig.ProducerDelay = map[string]uint64{
		"0": 16,
		"8": 4,
	}
	assert.Equal(t, testZenaConfig.CalculateProducerDelay(0), uint64(16))
	assert.Equal(t, testZenaConfig.CalculateProducerDelay(8), uint64(4))
	assert.Equal(t, testZenaConfig.CalculateProducerDelay(9), uint64(4))
}
...
func InitMinerSprintLength(genesis *core.Genesis, privKey *ecdsa.PrivateKey, withoutIris bool) (*node.Node, *eth.Zenanet, error) {
