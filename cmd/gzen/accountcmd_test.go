...
`
	...
	{
		gzen := runGzen(t, "account", "list", "--datadir", datadir)
		gzen.Expect(want)
		gzen.ExpectExit()
	}
	{
		gzen := runGzen(t, "--datadir", datadir, "account", "list")
		gzen.Expect(want)
		gzen.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	t.Parallel()
	gzen := runGzen(t, "account", "new", "--lightkdf")
	defer gzen.ExpectExit()
	gzen.Expect(`
...
func TestAccountHelp(t *testing.T) {
	...
	gzen = runGzen(t, "account", "import", "-h")
	gzen.WaitExit()
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gzen := runGzen(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gzen.ExpectExit()
	gzen.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	t.Parallel()
	gzen := runGzen(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gzen.ExpectExit()
	gzen.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gzen.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	t.Parallel()
	gzen := runMinimalGzen(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gzen.StderrText(), m) {
...
// https://github.com/zenanetwork/go-zenanet/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gzen.SetTemplateFunc("keypath", func(file string) string {
