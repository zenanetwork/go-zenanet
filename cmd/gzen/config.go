...
func loadBaseConfig(ctx *cli.Context) gzenConfig {
	...
	if ctx.IsSet(utils.ZenaMainnetFlag.Name) {
		setDefaultZenaMainnetGzenConfig(ctx, &cfg)
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gzenConfig) {
	...
	// Set Zena config flags
	utils.SetZenaConfig(ctx, &cfg.Eth)
...
func setDefaultMumbaiGzenConfig(ctx *cli.Context, config *gzenConfig) {
	...
	config.Node.IPCPath = utils.MakeDataDir(ctx) + "/zena.ipc"
	config.Node.HTTPModules = []string{"eth", "net", "web3", "txpool", "zena"}
...
func setDefaultZenaMainnetGzenConfig(ctx *cli.Context, config *gzenConfig) {
	config.Node.P2P.ListenAddr = fmt.Sprintf(":%d", 30303)
	config.Node.HTTPHost = "0.0.0.0"
	config.Node.HTTPVirtualHosts = []string{"*"}
	config.Node.HTTPCors = []string{"*"}
