...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 zen` in value to maximum `1 zen` per 24h period
...
implemented for `gzen`. There are no known security vulnerabilities in it, nor have we had any security problems with it so far.
