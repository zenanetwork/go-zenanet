...
import (
	...
	bparams "github.com/zenanetwork/go-zenanet/beacon/params"
	...
	"github.com/zenanetwork/go-zenanet/crypto/kzg4844"
	...
	"github.com/zenanetwork/go-zenanet/ethdb/remotedb"
	"github.com/zenanetwork/go-zenanet/ethstats"
	"github.com/zenanetwork/go-zenanet/graphql"
	...
	"github.com/zenanetwork/go-zenanet/metrics/exp"
	"github.com/zenanetwork/go-zenanet/metrics/influxdb"
	...
	"github.com/zenanetwork/go-zenanet/p2p/nat"
	"github.com/zenanetwork/go-zenanet/p2p/netutil"
	"github.com/zenanetwork/go-zenanet/params"
	"github.com/zenanetwork/go-zenanet/rpc"
	"github.com/zenanetwork/go-zenanet/triedb"
	"github.com/zenanetwork/go-zenanet/triedb/hashdb"
	"github.com/zenanetwork/go-zenanet/triedb/pathdb"
)
...
var (
	...
	StateSchemeFlag = &cli.StringFlag{
		Name:     "state.scheme",
		Usage:    "Scheme to use for storing zenanet state ('hash' or 'path')",
		Category: flags.StateCategory,
	}
	...
	ZenaLogsFlag = &cli.BoolFlag{
		Name:  "zena.logs",
		Usage: "Enable zena logs retrieval",
	}
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in zen) that can be sent via the RPC APIs (0 = no cap)",
	...
	HttpHeaderFlag = &cli.StringSliceFlag{
		Name:     "header",
		Aliases:  []string{"H"},
		Usage:    "Pass custom headers to the RPC server when using --" + RemoteDBFlag.Name + " or the gzen attach console. This flag can be given multiple times.",
		Category: flags.APICategory,
	}
...
// for Gzen and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gzen account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// setZenbase retrieves the zenbase from the directly specified command line flags.
func setZenbase(ctx *cli.Context, cfg *ethconfig.Config) {
	if !ctx.IsSet(MinerZenbaseFlag.Name) {
		return
	}

	addr := ctx.String(MinerZenbaseFlag.Name)
	...
	if err != nil || len(b) != common.AddressLength {
		Fatalf("-%s: invalid pending block producer address %q", MinerZenbaseFlag.Name, addr)
		return
	}

	cfg.Miner.Zenbase = common.BytesToAddress(b)
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.Bool(DeveloperFlag.Name):
		...
		// setZenbase has been called above, configuring the miner address from command line flags.
		if cfg.Miner.Zenbase != (common.Address{}) {
			developer = accounts.Account{Address: cfg.Miner.Zenbase}
		...
		cfg.Miner.Zenbase = developer.Address
...
// RegisterEthService adds an Zenanet client to the stack.
// The second return value is the full node instance.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Zenanet) {
	backend, err := eth.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Zenanet service: %v", err)
	}
	stack.RegisterAPIs(tracers.APIs(backend.APIBackend))
	return backend.APIBackend, backend
}

// RegisterEthStatsService configures the Zenanet Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Zenanet Stats service: %v", err)
	}
}
...
func RegisterFilterAPI(stack *node.Node, backend ethapi.Backend, ethcfg *ethconfig.Config) *filters.FilterSystem {
	...
	filterAPI := filters.NewFilterAPI(filterSystem, ethcfg.ZenaLogs)
	stack.RegisterAPIs([]rpc.API{{
		Namespace: "eth",
		Service:   filters.NewFilterAPI(filterSystem, ethcfg.ZenaLogs),
	}})
...
func RegisterFullSyncTester(stack *node.Node, eth *eth.Zenanet, target common.Hash) {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gzen.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gzen.", tagsMap)
		}
...
func DialRPCWithHeaders(endpoint string, headers []string) (*rpc.Client, error) {
	...
	if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gzen < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
