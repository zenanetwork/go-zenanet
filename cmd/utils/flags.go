...
func setZenbase(ctx *cli.Context, cfg *ethconfig.Config) {
	if ctx.IsSet(MinerZenbaseFlag.Name) {
		log.Warn("Option --miner.zenbase is deprecated as the zenbase is set by the consensus client post-merge")
	}
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (*eth.EthAPIBackend, *eth.Zenanet) {
