...
// nolint : paralleltest
func TestGenerateBlockAndImportZena(t *testing.T) {
	testGenerateBlockAndImport(t, false, true)
}

//nolint:thelper
func testGenerateBlockAndImport(t *testing.T, isClique bool, isZena bool) {
	var (
	...
	if isZena {
		chainConfig = *params.ZenaUnittestChainConfig

		engine, ctrl = getFakeZenaFromConfig(t, &chainConfig)
		defer ctrl.Finish()
	} else {
		if isClique {
...
func getFakeZenaFromConfig(t *testing.T, chainConfig *params.ChainConfig) (consensus.Engine, *gomock.Controller) {
	t.Helper()
	...
	contractMock := zena.NewMockGenesisContract(ctrl)
...
// nolint: paralleltest
// TestCommitInterruptExperimentZena_NormalFlow tests the commit interrupt experiment for Zena consensus by inducing
// an artificial delay at transaction level. It runs the normal mining flow triggered via new head.
func TestCommitInterruptExperimentZena_NormalFlow(t *testing.T) {
	// with 1 sec block time and 200 millisec tx delay we should get 5 txs per block
	testCommitInterruptExperimentZena(t, 200, 5, 0)

	time.Sleep(2 * time.Second)

	// with 1 sec block time and 100 millisec tx delay we should get 10 txs per block
	testCommitInterruptExperimentZena(t, 100, 10, 0)
}

// nolint : paralleltest
// TestCommitInterruptExperimentZenaContract tests the commit interrupt experiment for Zena consensus by inducing an artificial delay at OPCODE level.
func TestCommitInterruptExperimentZenaContract(t *testing.T) {
	// pre-calculated number of OPCODES = 123. 7*123=861 < 1000, 1 tx is possible but 2 tx per block will not be possible.
	testCommitInterruptExperimentZenaContract(t, 0, 1, 7)
	time.Sleep(2 * time.Second)
	...
	testCommitInterruptExperimentZenaContract(t, 0, 3, 2)
	time.Sleep(2 * time.Second)
	// pre-calculated number of OPCODES = 123. 3*123=369 < 1000, 2 tx is possible but 3 tx per block will not be possible.
	testCommitInterruptExperimentZenaContract(t, 0, 2, 3)
}

// nolint : thelper
// testCommitInterruptExperimentZenaContract is a helper function for testing the commit interrupt experiment for zena consensus.
func testCommitInterruptExperimentZenaContract(t *testing.T, delay uint, txCount int, opcodeDelay uint) {
	var (
		engine      consensus.Engine
		chainConfig *params.ChainConfig
		txInTxpool  = 100
	...
	engine, _ = getFakeZenaFromConfig(t, chainConfig)
...
// // nolint : thelper
// testCommitInterruptExperimentZena is a helper function for testing the commit interrupt experiment for zena consensus.
func testCommitInterruptExperimentZena(t *testing.T, delay uint, txCount int, opcodeDelay uint) {
	var (
		engine      consensus.Engine
		chainConfig *params.ChainConfig
		db          = rawdb.NewMemoryDatabase()
...
// TestCommitInterruptExperimentZena_NewTxFlow tests the commit interrupt experiment for zena consensus by inducing
...
func TestCommitInterruptExperimentZena_NewTxFlow(t *testing.T) {
	var (
		engine      consensus.Engine
		chainConfig *params.ChainConfig
		db          = rawdb.NewMemoryDatabase()
...
func BenchmarkZenaMining(b *testing.B) {
	chainConfig := params.ZenaUnittestChainConfig
	...
	blockPeriod, ok := back.genesis.Config.Zena.Period["0"]
	if !ok {
		blockPeriod = 1
	}
...
// params.ZenaUnittestChainConfig contains the NapoliBlock as big.NewInt(5), so the first 4 blocks will not have metadata.
// nolint: gocognit
func BenchmarkZenaMiningBlockSTMMetadata(b *testing.B) {
	chainConfig := params.ZenaUnittestChainConfig
