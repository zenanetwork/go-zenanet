...
type worker struct {
	...
	// large value. A large timeout allowance may cause Gzen to fail creating
...
// setZenbase sets the zenbase used to initialize the block coinbase field.
func (w *worker) setZenbase(addr common.Address) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.coinbase = addr
}

// zenbase retrieves the configured zenbase address.
func (w *worker) zenbase() common.Address {
	w.mu.RLock()
	defer w.mu.RUnlock()
...
func (w *worker) commitWork(interrupt *atomic.Int32, noempty bool, timestamp int64) {
	...
	if w.IsRunning() {
		...
		if coinbase == (common.Address{}) {
			log.Error("Refusing to mine without zenbase")
			return
		}
	}
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.IsRunning() {
		...
		// https://github.com/zenanetwork/go-zenanet/issues/24299
		env := env.copy()
		...
		case w.taskCh <- &task{receipts: env.receipts, state: env.state, block: block, createdAt: time.Now()}:
			...
			feesInZen := new(big.Float).Quo(new(big.Float).SetInt(fees), big.NewFloat(params.Zen))
			log.Info("Commit new sealing work", "number", block.Number(), "sealhash", w.engine.SealHash(block.Header()),
				"txs", env.tcount, "gas", block.GasUsed(), "fees", feesInZen,
