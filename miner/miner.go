...
type Config struct {
	Zenbase           common.Address `toml:",omitempty"` // Public address for block mining rewards
...
var DefaultConfig = Config{
	GasCeil:  30_000_000,
	GasPrice: big.NewInt(params.ZenaDefaultMinerGasPrice), // enforces minimum gas price of 25 gwei in zena

	// The default recommit time is chosen as two seconds since
	// consensus-layer usually will wait a half slot of time(6s)
	// for payload generation. It should be enough for Gzen to
	// run 3 rounds.
...
func (miner *Miner) SetZenbase(addr common.Address) {
	miner.worker.setZenbase(addr)
}
