...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gzen is "gzen". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gzen", keep that.
	if name == "gzen" || name == "gzen-testnet" {
		name = "Gzen"
	}
...
// These resources are resolved differently for "gzen" instances.
var isOldGzenResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gzen 1.4 are used if they exist.
	if warn, isOld := isOldGzenResource[path]; isOld {
		oldpath := ""
		if c.name() == "gzen" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGzenResourceWarning {
				c.oldGzenResourceWarning = true

				log.Warn("Using deprecated resource file, please move this file to the 'gzen' subdirectory of datadir.", "file", oldpath)
			}
...
func (c *Config) GetKeyStoreDir() (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-zenanet-keystore")
		isEphemeral = true
	}
