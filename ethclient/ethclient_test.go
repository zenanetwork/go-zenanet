...
// Verify that Client implements the zenanet interfaces.
var (
	_ = zenanet.ChainReader(&ethclient.Client{})
	_ = zenanet.TransactionReader(&ethclient.Client{})
	_ = zenanet.ChainStateReader(&ethclient.Client{})
	_ = zenanet.ChainSyncReader(&ethclient.Client{})
	_ = zenanet.ContractCaller(&ethclient.Client{})
	_ = zenanet.GasEstimator(&ethclient.Client{})
	_ = zenanet.GasPricer(&ethclient.Client{})
	_ = zenanet.LogFilterer(&ethclient.Client{})
	_ = zenanet.PendingStateReader(&ethclient.Client{})
	// _ = zenanet.PendingStateEventer(&ethclient.Client{})
	_ = zenanet.PendingContractCaller(&ethclient.Client{})
)
...
func newTestBackend(config *node.Config) (*node.Node, []*types.Block, error) {
	...
	// Create Zenanet Service
	ecfg := &ethconfig.Config{Genesis: genesis, RPCGasCap: 1000000}
	ethservice, err := eth.New(n, ecfg)
	if err != nil {
		return nil, nil, fmt.Errorf("can't create new zenanet service: %v", err)
	}
...
func testTransactionInBlock(t *testing.T, client *rpc.Client) {
	...
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != zenanet.NotFound {
		t.Fatal("error should be zenanet.NotFound")
	}
...
func ExampleRevertErrorData() {
	...
	call := zenanet.CallMsg{To: &contract, Gas: 30000}
