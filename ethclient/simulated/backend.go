...
type Client interface {
	zenanet.BlockNumberReader
	zenanet.ChainReader
	zenanet.ChainStateReader
	zenanet.ContractCaller
	zenanet.GasEstimator
	zenanet.GasPricer
	zenanet.GasPricer1559
	zenanet.FeeHistoryReader
	zenanet.LogFilterer
	zenanet.PendingStateReader
	zenanet.PendingContractCaller
	zenanet.TransactionReader
	zenanet.TransactionSender
	zenanet.ChainIDReader
}
...
// other code that interacts with the Zenanet chain.
type Backend struct {
	node   *node.Node
...
func NewBackend(alloc types.GenesisAlloc, options ...func(nodeConf *node.Config, ethConf *ethconfig.Config)) *Backend {
	// Create the default configurations for the outer node shell and the Zenanet
	...
	// Assemble the Zenanet stack to run the chain with
